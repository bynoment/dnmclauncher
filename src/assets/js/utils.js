...
async function addAccount(data) {
    ...
    // Kullanýcý adýyla minotar.net'ten skin baslýgýný al
    if (data.name) {
        let username = encodeURIComponent(data.name);
        let skinUrl = `https://minotar.net/avatar/${username}`;
        skin = skinUrl;
    }
    ...
    // Oyuncu basý ekleyelim
    let playerHeadDiv = document.createElement("div");
    
    div.querySelector(".profile-image").appendChild(playerHeadDiv);
...
async function setStatus(opt) {
    ...
    if (!opt) {
        ...
        statusServerElement.innerHTML = `Restart - 0 ms`;
    ...
    if (!statusServer.error) {
        ...
        statusServerElement.innerHTML = `Sunucu Durumu - ${statusServer.ms} ms`;
